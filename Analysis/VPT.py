...
class PromptedTransformer(Transformer):
    ...
    def incorporate_prompt(self, x):
        ...
        print(f"Input data shape: {x.size()}===================================")
        # Input data shape: torch.Size([64, 3, 224, 224])===================================     B * C * H * W
        B = x.shape[0]
        # after CLS token, all before image patches
        x = self.embeddings(x)  # (batch_size, 1 + n_patches, hidden_dim)
        print(f"Input embedded patch shape: {x.size()}===================================")
        # Input embedded patch shape: torch.Size([64, 197, 768])=================================== 
        prompt = self.prompt_proj(self.prompt_embeddings)
        print(f"Input projected prompt shape: {prompt.size()}===================================")
        # Input projected prompt shape: torch.Size([1, 100, 768])===================================  
        prompt = prompt.expand(B, -1, -1)
        print(f"Input expanded prompt shape: {prompt.size()}===================================")
        # Input expanded prompt shape: torch.Size([64, 100, 768])=================================== 
        prompt = self.prompt_dropout(prompt)
        print(f"Input dropout prompt shape: {prompt.size()}===================================")
        # Input dropout prompt shape: torch.Size([64, 100, 768])=================================== 
        x = torch.cat((
                x[:, :1, :],
                prompt,
                x[:, 1:, :]
        ...
        print(f"Output prompt + patch shape: {x.size()}===================================")
        # Output prompt + patch shape: torch.Size([64, 297, 768])=================================== 
        return x
